        ...
        try:
            ...
            if mode == 'analyze':
                prompt = (
                    ...
                    'For each distinct scene or key visual moment, write a detailed text-to-image prompt that could recreate '
                    'that exact frame in an AI image generator (Midjourney, DALL-E, Stable Diffusion, Canva AI, etc.). '
                    'Format as a numbered list. Each prompt MUST include all of the following elements:\n\n'
                    '1. **Subject**: appearance, clothing, skin tone, hair, expression, pose, and hand/body gesture at that exact moment.\n'
                    '2. **Expression realism**: describe the expression as natural and believable — '
                    'no overly exaggerated eyebrows, eyes, or mouth. If enthusiasm is present, describe it as "subtle warmth" or '
                    '"a natural engaged smile", not theatrical or performative.\n'
                    '3. **Camera angle & POV**: be very specific — e.g. selfie-style POV shot held at arm\'s length, '
                    'slightly tilted handheld angle, eye-level talking-head, low-angle looking up at subject, '
                    'overhead flat-lay, over-the-shoulder, slight Dutch tilt, etc. Match what is actually visible in the frame.\n'
                    '4. **Framing & shot type**: extreme close-up, close-up, medium close-up, medium shot, medium wide, wide shot — '
                    'and what part of the subject is in frame.\n'
                    '5. **Lighting**: describe the lighting setup precisely — e.g. soft natural window light from the left, '
                    'ring light reflected in eyes, warm golden hour backlight, cool overcast outdoor light, '
                    'harsh direct flash, soft diffused studio light, neon accent light, etc. '
                    'Note if shadows are soft or hard, and overall brightness/exposure feel.\n'
                    '6. **Background & setting**: what is behind the subject, depth of field (sharp background vs. blurred bokeh), '
                    'any visible props or environment details.\n'
                    '7. **Camera & lens style**: always end every prompt with the camera it appears to be shot on — '
                    'e.g. "shot on iPhone 11, wide-angle front camera" or "shot on iPhone 15 Pro, cinematic mode" or '
                    '"shot on Sony ZV-1, f/1.8 bokeh" — infer the most likely device from the video quality, grain, and framing style.\n'
                    '8. **Visual style keywords**: photorealistic, candid, vertical video format, shallow depth of field, '
                    'film grain, high contrast, muted tones, vivid saturation — whatever matches the actual footage.\n\n'
                    'Make each prompt fully self-contained. Someone with zero context must be able to paste it into any image AI '
                    'and generate a near-identical frame.'
                )
            else:
                prompt = (
